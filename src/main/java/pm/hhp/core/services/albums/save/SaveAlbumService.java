...
public class SaveAlbumService implements Service<AlbumRequest, AlbumResponse> {
  private AlbumRepository repository;

  private AlbumFactory factory;

  public SaveAlbumService(AlbumRepository repository, AlbumFactory factory) {
  ...
  public AlbumResponse execute(AlbumRequest request) {
    Album album = factory.getAlbumEntity(
            request.getAlbumId().orElse(null),
            request.getGroupId(),
            request.getName(),
